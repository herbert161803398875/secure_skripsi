...
Dqx       = linspace(0.00, 3.1, 20);
n         = length(Dqx);           % sudah ada
...
labelsCB = string(Dqx);           % label untuk color-bar
figure;
...
% Labels and title
title('Eigenvalue Loci Plot-Droop : Droop Gain $D_p$ Variation (P = 1.0 p.u.)', ...
      'Interpreter', 'latex', ...
      'FontSize', 14);
...
inset0 = axes('Position', [0.21 0.30 0.25 0.3]); % [left bottom width height]
box on;
hold on;
for k = 1:n
...
% 
% % [x y width height], normalized units
% inset1 = axes('Position', [0.15 0.55 0.25 0.3]); % [left bottom width height]
% box on;
% hold on;
% for k = 1:n
%     plot(real(eigen_all{k}), imag(eigen_all{k}), 'x', ...
