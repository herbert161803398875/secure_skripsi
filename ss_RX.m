...
%% Eigen Loci Things :
RXGX      = linspace(0.0, 2, 20);
n         = length(RXGX);           % sudah ada
cmap      = jet(n);                  % ⬅️ buat colormap (atau lines(n))
eigen_all = cell(n,1);               % penampung eigenvalue tiap titik
% (opsional) jika ingin legend warna kontinu berbasis Lg_pu:
labelsCB = string(RXGX);           % label untuk color-bar
figure;
Lg_pu = 0.5;
...
for idx = 1:n
    ...
    %     % 1. Pastikan seluruh variabel & IC sudah ada di workspace
    % set_param('test_ss_infload1','SimulationCommand','update');   % refresh mask
    
    % 2. Dapatkan operating-point lalu linearise
    modelName = 'test_ss_infload1';
    load_system(modelName);
    opPoint = operpoint(modelName);
    io  = getlinio(modelName);
    G   = linearize(modelName,opPoint,io);
    [A,B,C,D] = ssdata(G);
    % ---------- eigen & participation ----------
    [V,Dr] = eig(A);
    lambda = diag(Dr);
    ...
    eigen = eig(A);
    eigen_all{idx}=eigen;
    plot(eigen, 'x', 'MarkerSize', 10, 'LineWidth', 1.25, 'Color', cmap(idx,:));
    hold on;
end

% Labels and title
title('Eigenvalue Loci Plot-Droop : $\frac{R}{X}$ Variation (P = 1.0 p.u.)', ...
      'Interpreter', 'latex', ...
      'FontSize', 14);
xlabel('Real Part', 'Interpreter', 'latex');
ylabel('Imaginary Part', 'Interpreter', 'latex');
% title('Eigenvalue Evolution Over Iterations');
grid on;
axis tight;

colormap(cmap);
cb = colorbar;
cb.Label.Interpreter = 'latex';  % Enable LaTeX formatting
cb.Label.String = '$\frac{R}{X}$';
cb.Label.FontWeight = 'bold';          % Make label bold
cb.Label.FontSize = 15;         
cb.Label.Position = [0.5, 1.05, 0];   % Move label above the colorbar (tweak Y value if needed)
cb.Label.Rotation = 0;             % Set label horizontal (rotation = 0)
cb.Label.HorizontalAlignment = 'center'; % Center alignment
cb.Ticks      = linspace(0,1,n);
cb.TickLabels = labelsCB;           % tampilkan nilai Lg_pu
...
% [x y width height], normalized units
inset0 = axes('Position', [0.15 0.35 0.25 0.3]); % [left bottom width height]
box on;
hold on;
for k = 1:n
    plot(real(eigen_all{k}), imag(eigen_all{k}), 'x', ...
         'Color', cmap(k,:), 'MarkerSize', 10, 'LineWidth', 1.5);
end
